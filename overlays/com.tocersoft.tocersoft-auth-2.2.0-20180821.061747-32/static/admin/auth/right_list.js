...
function deleteSelectNode(nodeId){
	...
	$.post(basePath+"/admin/auth/right/del.htm?m="+Math.random(),{"selIds":nodeId,"item.parentId":curNode.id},function(data){
		$("body",parent.document).unmask();
...
function resizeWidth(){
	...
	$("#categoryContentDiv").width(ww-tw);
...
function addChild(){
	...
	$.dialog.open(
		basePath+"/admin/auth/right/edit.htm?item.parentId="+curNode.id+"&item.location="+location,{
			title : "新增资源标签",
			width : "700px",
			height : "500px",
			lock : true
...
function editNode(){
	...
	$.dialog.open(basePath+"/admin/auth/right/edit.htm?item.id="+curNode.id,{
			title : "修改资源标签",
			width : "700px",
			height : "500px",
			lock : true
...
function treeLoadUrl(treeId, treeNode) {
	...
	return treeNode == null
			? basePath+"/admin/auth/right/listRightByTree.htm?pid=0"
			: basePath+"/admin/auth/right/listRightByTree.htm?pid="+treeNode.id;
}
/**
 * 右键点击事件
 * @param event
 ...
 */
function addParent(){
	$.dialog.open(basePath+"/admin/auth/right/edit.htm?item.parentId=0&item.location=1",{
		title : "新增资源标签",
		width : "700px",
		height : "500px",
		lock : true
