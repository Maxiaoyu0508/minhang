...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='left' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			//李宁 20171212增加当前选中行位置
			var scrollPosition = $("#"+this.id).closest(".ui-jqgrid-bdiv").scrollTop();
			selectedScrollArray[this.id] = scrollPosition;
			//李宁  20171212将当前表格最新选择的行保存，历史选择行清空
			var checked = $("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+ri).is(":checked");
			var keyId = ts.id+"+"+ri;
			if(checked){
...
(function($){
	 ...
	 */
	/*
	//http%3A%2F%2Flocalhost%3A8081%2Ftocersoft-product%2Fadmin%2Fproduct%2Findex.htm__table1=4
	var thisLocation = encodeURIComponent(window.location.href);
	var cookieArray = document.cookie.split(";");
	$.each(cookieArray,function(i,v){
		 if(v.indexOf("__")!=-1){
			var ol = v.split("=")[0];
			if(ol.indexOf(thisLocation)!=-1){
				var p =  v.split("=")[1];
				var transPage = p;
				var tId = ol.split("__")[1];
				$("table").each(function(){
	...
	*/
	...
	$.extend($.jgrid.defaults,{
		...
		loadError:function(xhr,status,error){
			//$.dialog.alert("网络繁忙，请稍后重试");
		},
		gridComplete:function(){
			var winWidth = $(window).width() - 10;
			$(this).setGridWidth(winWidth);
			// 李宁20171208刷新回掉
		  // 设置表格历史滚动条定位
		  setHistoryTop(this.id);
		  // 设置表格历史选择再次选中
		  setHistorySelect(this.id);
		  // 设置表格历史选择再次选中
		  //setUpDown(this.id);
		}
	});
 ...
 * @returns
 */
function setHistoryTop(tableId) {
  for (var key in selectedScrollArray) {
    if (key == tableId) {
      var value = selectedScrollArray[key];
      if (value != 0) {
        //console.log("key="+key+" value="+value);
        $("#" + key).closest(".ui-jqgrid-bdiv").scrollTop(value);
        break;
      }
    }
  }
 ...
 * @returns
 */
function setHistorySelect(tableId) {
  for (var i = 0; i < selectedIdArray.length; i++) {
    var obj = selectedIdArray[i];
    if (obj != "remove") {
      var key = obj.split("+")[0];
      if (key == tableId) {
        //console.log("i="+i+" key="+key+" value="+obj.split("+")[1])
        $("#" + key).setSelection(obj.split("+")[1]);
      }
    }
  }
}
 ...
 * @returns
 */
function setUpDown(tableId) {
  $("#" + tableId).focus();
  console.log(tableId + "位置=" + $.inArray(tableId, keyDownTableArray));
  if ($.inArray(tableId, keyDownTableArray) == -1) {
    keyDownTableArray.push(tableId);
    $("#" + tableId).keydown(jqgridKeyDownEvent);
  }
}
 ...
 */
// 绑定过键盘事件的TABLE
var keyDownTableArray = new Array();

function jqgridKeyDownEvent(e) {
  var code = e.keyCode;
  var target = e.target;
  var jqgrid = e.currentTarget;
  var tableId = jqgrid.id;
  var gridObj = $("#" + tableId);
  var rowId = $("#" + tableId).jqGrid('getGridParam', 'selrow');
  var newSelId = rowId;
  var scrollPosition = gridObj.closest(".ui-jqgrid-bdiv").scrollTop();
  console.log(target);
  console.log("tableId=" + tableId + "  rowId=" + rowId);
  if (code == 38 || code == 40) {
    if (code == 38) {
      // 由于向上有标题行，所以判断ID是否未空
      var lastObj = $("#" + tableId + " #" + rowId).prev()[0];
      ...
      // 由于向下没有行则判断对象是否underfined
      var nextObj = $("#" + tableId + " #" + rowId).next()[0];
    ...
    gridObj.jqGrid("setSelection", newSelId);
    gridObj.closest(".ui-jqgrid-bdiv").scrollTop(scrollPosition - 10);
  }
}