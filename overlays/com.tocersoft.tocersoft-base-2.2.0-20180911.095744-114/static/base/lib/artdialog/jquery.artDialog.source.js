...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
		var that = this, DOM,
			...
			: { backgroundImage: "url('" + basePath + "/static/base/lib/artdialog/skins/icons/" + icon + ".png')"
            });
  ...
  max: function () {
    ...
    if (max.attr('state') == 'false') {  // 判断是否已最大化
      ...
      jQuery(DOM.border[0]).parent().parent().css('width', 'auto');  // 修改窗口最外层 div 定义的宽度，这个 div 是窗口外层的样式窗口，比我们定义的窗口宽上 30 像素，根据使用的皮肤不同有所区别

      if(max.attr('_width')<='150'){	// 小于150的视为tips弹窗
        this.size('auto','auto');
      }else{
...
// 触发浏览器预先缓存背景图片
_$window.bind(_path +'/static/base/lib/artdialog/skins/icons/loading.gif', function () {
	setTimeout(function () {
		if (_count) return;
...
+                                            '<div class="aui_max" state="false" style="display: none;"></div>'
