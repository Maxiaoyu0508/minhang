    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    (this.includeDem('district') ? '<a data-count="district">区县</a>' : '') + '<button type="button" class="btn btn-danger">确定</button></div>' +
        ...
        bind: function () {
            ...
            this.$dropdown.on('click', '.city-select a', function () {
                ...
                if ($active.data('code') !== $(this).data('code') || $($select).hasClass("province")) {
                    $select.data('item', {
                    ...
                    $this.feedText();
                    $this.feedVal();
                }else if(!$($select).hasClass("province")){
                	$(this).removeClass('active');
                	$($select).next().find("a.active").removeClass('active');
                	//取消选择
                	$select.removeData('item');
                	$($select).next().removeData('item');
                    $this.feedText();
                    $this.feedVal();
                }
        ...
        getProvinceList: function (data) {
            ...
            $.each(data, function (i, n) {
                ...
                list.push(
                    ...
                    (n.selected ? ' active' : '') +
                    '">' +
                    ( simple ? $this.simplize(n.address, PROVINCE) :n.address) +
                    '</a>');
                list.push('</dd></dl>');
            });
