...
(function () {
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            ...
            /*
             * 按照选择的顺序排列
             * edited by zhangqiang at 2018-06-25 11:38
             */
            var uploadFileList = this.uploader.getFiles();
            for(var n=0;n<uploadFileList.length;n++){
              var curUploadFile = uploadFileList[n];
              for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];
                if(data.original == curUploadFile.name){
                  list.push({
                    src: prefix + data.url,
