$(document).ready(function(){
	...
	$("#table").jqGrid({
	    ...
	    datatype:"json",
	    mtype:'post',
	    postData:{"condition.columnId":$("#columnId").val(),"condition.sectionId":$("#sectionId").val()},
...
function delArticle(){
	...
	var postData = {};
	postData["columnId"] = $("#columnId").val();
	postData["sectionId"] = $("#sectionId").val();
	postData["selIds"] = selIds.join(",");
	$.post(basePath+"/admin/cms/column/article/saveCmsColumnArticleList.htm?m="+Math.random(),postData,function(data){
		var win = $.dialog.open.origin;
		win.reloadTableBySection($("#sectionId").val());
		$.dialog.close();
	},"json");
}
//重新加载
