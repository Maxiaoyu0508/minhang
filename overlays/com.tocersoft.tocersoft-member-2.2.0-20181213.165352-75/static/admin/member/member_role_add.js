...
function initFormValidator(){
    ...
    $("#name").formValidator({validatorGroup:1}).inputValidator({min:1,max:255,onErrorMin:"会员角色名称不能为空",onErrorMax:"会员角色名称长度过长"});
    $("#code").formValidator({validatorGroup:1}).inputValidator({min:1,max:255,onErrorMin:"会员角色代码不能为空",onErrorMax:"会员角色代码长度过长"});
    //$("#remark").formValidator({validatorGroup:1}).inputValidator({min:1,max:255,onErrorMin:"说明不能为空",onErrorMax:"说明长度过长"});
}
...
function initTree(){
	...
	var rightUrl = basePath + "/admin/member/right/listRightByRoleId.htm?m="+Math.random();
