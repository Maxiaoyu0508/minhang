...
function initTable(){
	...
	$("#table").jqGrid({
		...
		colModel:[ 
            ...
            {name:"showSession",width:100,formatter:sessionFormater},
            {name:"id",hidden:true}
		],
		viewrecords: true,
...
function sessionFormater(cellvalue,options,rowObject){
    return '<a href="javascript:void(0);" onclick="showSession('+ rowObject.id +')">查看活动场次</a>'
}

function showSession(id) {
	$.dialog.data('activityId',id);
    $.dialog.open(basePath + "/admin/activity/session/index.htm?m="+Math.random(),{
        title : "查看活动场次",
        width : "1200px",
        height : "600px",
        lock : true
